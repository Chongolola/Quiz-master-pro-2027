  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[80vh] px-4">
      <motion.div 
        ...
        className="w-full max-w-md bg-card p-8 rounded-3xl border border-main shadow-2xl space-y-6"
      >
        ...
        {/* Mode Toggle */}
        <div className="flex bg-main p-1 rounded-2xl">
          <button 
            onClick={() => setMode('login')}
            className={`flex-1 py-2 rounded-xl text-sm font-bold transition-all ${mode === 'login' ? 'bg-indigo-600 text-white shadow-lg' : 'text-muted hover:text-main'}`}
          >
            Entrar
          </button>
          <button 
            onClick={() => setMode('register')}
            className={`flex-1 py-2 rounded-xl text-sm font-bold transition-all ${mode === 'register' ? 'bg-indigo-600 text-white shadow-lg' : 'text-muted hover:text-main'}`}
          >
            Cadastrar
          </button>
        </div>
          ...
          {mode === 'register' && (
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-main border border-main rounded-xl py-3 pl-10 pr-4 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all text-main"
              />
            </div>
          )}
        ...
        <div className="relative py-4">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-card px-2 text-muted">Ou continue como</span>
          </div>
        </div>
